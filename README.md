# R Programming Practicals

This repository contains my **Semester 7 R Programming practical assignments** completed as part of the **B.Tech Computer Science (AI & ML)** curriculum.

## Course Objectives

* Learn the fundamentals of R programming
* Perform data cleaning and preprocessing
* Handle missing values and exceptions
* Create reusable functions
* Visualize data using R
* Export processed datasets
* Apply R programming to real-world datasets

## Repository Structure

```text
R-Programming-Practicals/
│
├── Practical01/
├── Practical02/
├── Practical03/
├── Practical04/
├── Practical05/
├── Practical06/
├── Practical07/
├── Practical08/
├── Practical09/
└── Practical10/
```

## Practicals

| Practical    | Topic                             | Status      |
| ------------ | --------------------------------- | ----------- |
| Practical 1  | Air Quality Data Cleaning Using R | ✅ Completed |
| Practical 2  | Coming Soon                       | ✅ Completed          |
| Practical 3  | Coming Soon                       | ✅ Completed            |
| Practical 4  | Coming Soon                       | ✅ Completed           |
| Practical 5  | Coming Soon                       |  ✅ Completed       |
| Practical 6  | Coming Soon                       | ⏳           |
| Practical 7  | Coming Soon                       | ⏳           |
| Practical 8  | Coming Soon                       | ⏳           |
| Practical 9  | Coming Soon                       | ⏳           |
| Practical 10 | Coming Soon                       | ⏳           |

## Technologies Used

* **R**
* **RStudio**
* **Quarto**

## Author

**Tanvi Patil**
